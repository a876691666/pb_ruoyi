...
import { Alert, Input, Popconfirm, Space, Tooltip } from 'ant-design-vue';
...
function handleRowClick(row: DictType) {
  if (lastDictType.value === row.dict_type) {
    return;
  }
  currentRowId.value = row.id;
  emitter.emit('rowClick', row.dict_type);
}
...
watch(searchValue, (value) => {
  ...
  if (value) {
    const names = tableAllData.value.filter((item) =>
      item.dict_name.includes(searchValue.value),
    );
    const types = tableAllData.value.filter((item) =>
      item.dict_type.includes(searchValue.value),
    );
