 ...
 */
const onSelected: SelectHandler = async (id: string, option: any) => {
  if (unref(lastSelected) === id) {
  ...
  try {
    ...
    await tenantDynamicToggle(id);
    lastSelected.value = id;

    // 关闭之前的message 只保留一条
    messageInstance.value?.();
    messageInstance.value = message.success(
      `${$t('component.tenantToggle.switch')} ${option.company_name}`,
    );
...
function filterOption(input: string, option: TenantOption) {
  return option.company_name.toLowerCase().includes(input.toLowerCase());
}
</script>
...
<template>
  <div v-if="showToggle" class="mr-[8px] hidden md:block">
    <Select
      ...
      :disabled="loading"
      :field-names="{ label: 'company_name', value: 'id' }"
