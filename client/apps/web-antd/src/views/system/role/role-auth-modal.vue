...
async function setupDeptTree(id: number | string) {
  ...
  formApi.setFieldValue('dept_ids', resp.checkedKeys);
  // 设置菜单信息
...
async function handleConfirm() {
  try {
    ...
    if (data.data_scope === '2') {
      const dept_ids = deptSelectRef.value?.[0]?.getCheckedKeys() ?? [];
...
function handleCheckStrictlyChange(value: boolean) {
  formApi.setFieldValue('dept_check_strictly', value);
}
</script>
...
<template>
  <BasicModal class="min-h-[600px] w-[550px]" title="分配权限">
    <BasicForm>
      <template #dept_ids="slotProps">
        <TreeSelectPanel
          ref="deptSelectRef"
          v-bind="slotProps"
          :check-strictly="formApi.form.values.dept_check_strictly"
