...
pb.afterSend = function (response, data: { code: number; message: string }) {
  if (
    response.status !== 200 &&
    !(response.status === 204 && response.statusText === 'No Content')
  ) {
    if (response.url.includes('/auth-refresh')) {
      return handleUnauthorizedLogout();
    }
    if (response.url.includes('/auth-with-password')) {
      return message.error($t('authentication.failedLogin'));
    }
...
export const Ands = (args: any[]) => {
  if (args.length === 0) return '';
  let filterSql = '';
  args
    .filter((value) => value !== undefined && value !== null && value !== '')
    .forEach((item, index) => {
      filterSql += index === 0 ? `(${item})` : ` && (${item})`;
    });
  return filterSql;
};

export const Ors = (args: any[]) => {
  if (args.length === 0) return '';
  let filterSql = '';
  args
    .filter((value) => value !== undefined && value !== null && value !== '')
...
export const DateRange = (
  key: string,
  startAndEnd: ReturnType<typeof dayjs>[],
) => {
  const [start, end] = startAndEnd || [];
  if (!start || !end) {
    return '';
  }
  return `${key} >= "${start?.toISOString()}" && ${key} <= "${end?.toISOString()}"`;
};

export const SortForParams = (params?: PageQuery) => {
  // 支持多字段排序：两个参数均可能为逗号分隔的字符串
  // 示例：
  // isAsc: "asc", orderByColumn: "id,create_time" => "id create_time"
  // isAsc: "desc", orderByColumn: "id,create_time" => "-id -create_time"
  // isAsc: "asc,desc", orderByColumn: "id,create_time" => "id -create_time"
  const columns = params?.orderByColumn
    ?.split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const dirs = params?.isAsc
    ?.split(',')
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);

  if (columns?.length && dirs?.length) {
    return columns
      .map((col, idx) => {
        const dir = dirs[idx] ?? dirs[0]; // 当方向数量与列数量不一致时，使用第一个方向作为默认
...
export const DictTrans = (obj: Record<string, any>, action = LIKE) => {
