...
async function handleOpenChange(open: boolean) {
  ...
  // 外部的role_ids post_ids才是真正对应的  新增时为空
  ...
  const postNames = posts
    .filter((item) => post_ids.includes(item.id))
    .map((item) => item.post_name);

  const roleNames = roles
    .filter((item) => role_ids.includes(item.id))
    .map((item) => item.role_name);

  (user as UserWithNames).postNames = postNames;
  (user as UserWithNames).roleNames = roleNames;
  if (user?.create_time) {
    (user as UserWithNames).create_time = dayjs(user.create_time).format(
      'YYYY-MM-DD HH:mm:ss',
    );
  }
  // 赋值
...
const mixInfo = computed(() => {
  ...
  const { dept_name, nick_name, user_name } = currentUser.value;
  return `${user_name} / ${nick_name} / ${dept_name ?? '-'}`;
});
...
const diffLoginTime = computed(() => {
  ...
  const { login_date } = currentUser.value;
  // 默认en显示
  dayjs.locale('zh-cn');
  // 计算相差秒数
  const diffSeconds = dayjs().diff(dayjs(login_date), 'second');
  /**
   * 转为时间显示(x月 x天)
  ...
  <BasicModal :footer="false" :fullscreen-button="false" title="用户信息">
    <Descriptions v-if="currentUser" size="small" :column="1" bordered>
      ...
      <DescriptionsItem label="上次登录时间">
        <span>{{ currentUser.login_date ?? '-' }}</span>
        <Tag
          class="ml-2"
          v-if="diffLoginTime"
          :bordered="false"
