...
import { generateCodeFromGenInfo } from './index';
...
const [BasicModal, modalApi] = useVbenModal({
  async onOpenChange(isOpen) {
    ...
    const options: any = modalApi.getData();
    const data = generateCodeFromGenInfo(options, true);
  ...
  >
    <div v-if="currentCodeData" class="flex gap-[8px]">
      <div class="h-[calc(100vh-80px)] w-[300px] overflow-y-scroll">
        ...
        <Alert class="mt-2" show-icon message="👆显示的名称最终下载文件名" />
      </div>
      <CodeMirror
        v-model="codeContent"
        :language="language"
