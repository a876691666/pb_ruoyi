 ...
 * @returns 返回 shallowRef，且相同 dictName 返回同一个 shallowRef
 */
// 缓存每个 dictName 对应的 shallowRef，确保同名返回同一引用
const dictOptionsRefMap = new Map<string, ShallowRef<DictOption[]>>();

export function getDictOptions(
  dictName: string,
  formatNumber = false,
): ShallowRef<DictOption[]> {
  const {
    dictRequestCache,
    setDictInfo,
    getDictOptions: getStoreDictOptions,
  } = useDictStore();
  const dataList = getStoreDictOptions(dictName);

  // 初始化或复用 shallowRef
  let ref = dictOptionsRefMap.get(dictName);
  if (!ref) {
    const created = shallowRef<DictOption[]>(dataList);
    dictOptionsRefMap.set(dictName, created);
    ref = created;
  }
    ...
    dictRequestCache.set(
      ...
      dictDataInfo(dictName)
        .then((resp) => {
          ...
          ref.value = getStoreDictOptions(dictName);
        })
        .catch((error) => {
          /**
  ...
  return ref as ShallowRef<DictOption[]>;
}
