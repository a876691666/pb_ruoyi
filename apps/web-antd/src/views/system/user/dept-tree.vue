      ...
      >
        ...
        <div class="h-full overflow-x-hidden px-[8px]">
          <Tree
            ...
            :class="$attrs.class"
            :field-names="{ title: 'dept_name', key: 'id' }"
            ...
            @select="$emit('select')"
          >
            <template #title="{ dept_name }">
              <span v-if="dept_name.includes(searchValue)">
                {{ dept_name.substring(0, dept_name.indexOf(searchValue)) }}
                <span class="text-primary">{{ searchValue }}</span>
                {{
                  dept_name.substring(
                    dept_name.indexOf(searchValue) + searchValue.length,
                  )
                }}
              </span>
              <span v-else>{{ dept_name }}</span>
            </template>
          </Tree>
          <!-- 仅本人数据权限 可以考虑直接不显示 -->
