...
async function setupMenuTree(id?: number | string) {
  if (id) {
    ...
    await nextTick();
    await formApi.setFieldValue('menu_ids', resp.checkedKeys);
  } else {
    ...
    // keys依赖于menu 需要先加载menu
    await nextTick();
    await formApi.setFieldValue('menu_ids', []);
  }
}
...
async function customFormValueGetter() {
  ...
  // 获取勾选信息
  const menu_ids = menuSelectRef.value?.getCheckedKeys?.() ?? [];
  const mixStr = v + menu_ids.join(',');
  return mixStr;
}
...
function handleMenuCheckStrictlyChange(value: boolean) {
  formApi.setFieldValue('menu_check_strictly', value);
}
</script>
      ...
      <template #menu_ids="slotProps">
        <div class="h-[600px] w-full">
          ...
          <MenuSelectTable
            ...
            :association="formApi.form.values.menu_check_strictly"
            :menus="menuTree"
